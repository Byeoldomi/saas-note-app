                        ...
                        {history.map((record) => (
                            <tr key={record.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/30 transition-colors">
                                ...
                                <td className="px-6 py-4">
                                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${record.status === 'PAID' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' :
                                            record.status === 'FAILED' ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400' :
                                                'bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-400'
                                        }`}>
                                        {record.status === 'PAID' && <span className="material-icons-outlined text-[14px] mr-1">check_circle</span>}
                                        {record.status === 'FAILED' && <span className="material-icons-outlined text-[14px] mr-1">error</span>}
                                        {record.status}
                                    </span>
                                </td>
